...
func initUser() userservice.Client {
	...
	provider.NewOpenTelemetryProvider(
		provider.WithServiceName(conf.GlobalServerConfig.UserSrvInfo.Name),
	...
	c, err := userservice.NewClient(
		conf.GlobalServerConfig.UserSrvInfo.Name,
		client.WithResolver(r),                                     // service discovery
		client.WithLoadBalancer(loadbalance.NewWeightedBalancer()), // load balance
		client.WithMuxConnection(1),                                // multiplexing
		client.WithSuite(tracing.NewClientSuite()),
		client.WithClientBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: conf.GlobalServerConfig.UserSrvInfo.Name}),
	)
	if err != nil {
